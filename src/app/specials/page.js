...
const OffersMenu = () => {
    ...
    useEffect(() => {
      ...
      const saveOrder = async () => {
        const orderedMenu = menuItems.map((c, index) => ({ _id: c._id, order: index }));
          ...
          try {
            const response = await fetch('/api/offer-menu/updateOrder', {
              method: 'PUT',
              headers: {
  ...
  return (
    ...
    <div className='flex gap-4 justify-center items-center mt-5'>
        <Link href='/specials/new' className='bg-red-500 text-white flex gap-2 px-4 py-2 rounded-full'>Add Menu Item 
        <PlusCircle />
         </Link>
    </div>
