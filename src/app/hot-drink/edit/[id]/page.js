...
const EditTeaPage = () => {
    ...
    useEffect(() => {
    fetch('/api/tea-menu/updateOrder').then(response=>{response.json().then(data=>{
    ...
    const handleDeleteItem = async () => {
        const deletePromise = new Promise( async(resolve, reject) => {
          const response = await fetch('/api/tea-menu?_id='+id, {
                method: 'DELETE',
...
<div className='flex gap-32  items-center mt-5 text-center max-w-xl mx-auto'>
        <Link href='/hot-drink' className='bg-gray-500 hover:bg-red-600 text-white p-2 rounded-lg flex justify-start gap-2 text-center'>
            <ChevronLeftCircle />
            Back
        </Link>
    <h2 className='text-3xl font-bold text-center justify-center'>Tea Item</h2>
</div>
