...
const fetchCategories = async () => {
    fetch('/api/tea-categories').then(response=>{response.json().then(data=>{
...
const saveOrderSmoke = async () => {
  ...
  try {
    const response = await fetch('/api/tea-categories/updateOrder', {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
 ...
 const handleDelete = async (_id) => {
  const deletePromise = new Promise( async(resolve, reject) => {
   const response=   await fetch('/api/tea-categories?_id='+_id, {
         method: 'DELETE',
         headers: {
             'Content-Type': 'application/json'
         },
...
export default TeaCategoryAdmin